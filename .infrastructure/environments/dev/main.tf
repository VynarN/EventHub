...
# Subnets
# Delegated to Microsoft.Web/serverFarms for regional VNet integration (App Service + Functions).
# Azure does not allow service endpoints on the same subnet as this delegation; use private endpoints + private DNS for PaaS.
module "app_subnet" {
...
# Function host storage (created inside function_app module); stored in KV for CI/CD and tooling.
